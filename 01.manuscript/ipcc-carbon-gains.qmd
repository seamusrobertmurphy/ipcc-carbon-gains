...
This paper extends both. Wilson et al. performed the subtraction for inland organic soils at a single metric; Zhang et al. tested metric sensitivity for one country and one pathway; we run the subtraction across every pathway the Supplement supports, add the duration dimension that neither considered, and show that the metric test and the uncertainty test select the same rows. The increment is coverage and the joint treatment of metric, duration and offset, not the observation that a horizon matters.
