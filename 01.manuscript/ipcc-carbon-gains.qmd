...
## Emission factors for rewetted soils
...
ws_socref <- c(Boreal = 116, `Cold temperate dry` = 87, `Cold temperate moist` = 128,
               `Warm temperate dry` = 74, `Warm temperate moist` = 135,
...
A compiler reporting the GWP-100 point estimate, which is what UNFCCC inventory practice requires, discloses neither. No single threshold on the GWP-100 benefit separates the failing rows from the rest: they run from 1.26 to 6.24 t CO2e/ha/yr, and the row at 6.24 fails both tests while rows below it in the ordering pass. The quantity that decides the outcome is not the size of the benefit but its size relative to the rewetted methane flux, which is the term both tests are responding to.
...
## The nutrient fallback penalises the wrong country
...
Six limitations bound the result. The Monte Carlo resamples the drained carbon dioxide term and both rewetted terms; drained land and ditch methane, the ditch fraction and nitrous oxide carry no published interval in the source tables and are held at their defaults, so the reported probabilities understate total uncertainty and do so asymmetrically, tightening the drained side relative to the rewetted one. The draws are triangular on the published central value and 95% bounds, which treats a confidence interval as the support of the distribution and, on the strongly right-skewed methane intervals, places the sampled median above the published central value; the count of five of thirteen is robust to that choice, but the individual probabilities and medians are not, and should be read as ordinal rather than as calibrated. Indirect nitrous oxide is omitted from the drained side, which understates the benefit of rewetting and is therefore conservative with respect to our conclusion. The temperate Forest Land nutrient split is assigned rather than published, and that row is load-bearing. GWP\* is applied to two pathways above rather than across the ledger, so its effect on the remaining pathways is not established [@smith2021; @collins2020]. And the Irish results carry no propagated uncertainty at all: the country-specific factors are applied as point estimates, and three of the Irish conclusions are sign claims on differences small relative to the gross fluxes that produce them.

# Conclusions

A countable carbon gain is not a climate benefit, and the guidance that makes the gain countable supplies no way to tell the two apart. Assembling the Tier 1 and Tier 2 gain pathways on a single greenhouse-gas-weighted axis shows the gap is not marginal. Rewetting organic soils is beneficial on the point estimate in every Tier 1 default combination at GWP-100 but harmful in five of thirteen at GWP-20, and on Ireland's own adopted factors it is a penalty at both horizons; the probability of benefit falls below 0.95 in five of thirteen at GWP-100 alone, with the median at or below zero in four; inland wetland mineral soil rewetting books a positive stock change that its own methane factor exceeds in all nine climate regions for the first twenty years, with no counterfactual in Chapter 5 to offset against; a restored tidal marsh below 18 ppt is a net source in absolute terms while remaining a benefit of some 27 t CO2e/ha/yr against the drained state; and fertilised improved grassland is guaranteed to cross into net source because a twenty-year gain is set against a perpetual penalty.
